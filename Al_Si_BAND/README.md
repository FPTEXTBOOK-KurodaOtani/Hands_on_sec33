...
### Important note on the number of bands

For band-structure and density-of-states calculations, it is important to include a sufficient number of unoccupied states.

In the SCF output, the number of orbitals calculated by default is shown as the number of Kohn-Sham states. For example:

```text
bravais-lattice index     =            0
lattice parameter (alat)  =       5.3984  a.u.
unit-cell volume          =     111.2451 (a.u.)^3
...
Here, the SCF calculation uses 6 Kohn-Sham states.

However, when drawing a band structure or calculating the density of states, it is often necessary to plot not only the occupied states but also a sufficient number of unoccupied states. Therefore, in the non-self-consistent band or DOS calculation, it is recommended to explicitly increase the number of bands using `nbnd` in the `&SYSTEM` namelist.
...
The value of `nbnd` should be chosen large enough to include the unoccupied energy range of interest in the final band diagram or DOS plot.
...
Please note that `bands.out` fails to find the U point as a high-symmetry point.
...
6. The number of bands in the band or DOS calculation is sufficient to include the desired unoccupied states.
7. The x-axis labels in `band_plot_FCC.py` are consistent with the path:
   ```text
   L -> Gamma -> X -> U -> Gamma
   ```
