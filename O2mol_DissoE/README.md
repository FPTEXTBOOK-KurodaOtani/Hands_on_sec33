...
[
D_{\mathrm{O_2}} = 2E_{\mathrm{O}} - E_{\mathrm{O_2}}
]

where (E_{\mathrm{O}}) is the total energy of an isolated oxygen atom and (E_{\mathrm{O_2}}) is the total energy of the oxygen molecule.

## Important Note for the Isolated Oxygen Atom

When calculating the isolated oxygen atom, the size of the simulation cell must be chosen carefully.
...
Here, `nspin = 2` enables spin-polarized calculation.
The `starting_magnetization` values give an initial magnetic moment to the oxygen atoms, which helps the calculation converge to the correct spin-polarized state.
